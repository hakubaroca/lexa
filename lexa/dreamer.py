...
class Dreamer(nn.Module):
    ...
    def __init__(self, config, logger, dataset):
        ...
        self._metrics = collections.defaultdict(lambda: tools.MeanMetric())
        self._step = torch.tensor(count_steps(config.traindir), dtype=torch.int64)
        # Schedules
    ...
    def __call__(self, obs, reset, state=None, training=True):
        ...
        if state is not None and reset.any():
            mask = torch.tensor(1 - reset, dtype=self._float)[:, None]
            state = tools.apply_mask(state, mask)
        ...
        if training:
            ...
            self._logger.step = self._config.action_repeat * self._step.item()
            return action, state
        else:
    ...
    def _policy(self, obs, state, training, reset, should_expl=False):
        ...
        feat, latent = self._wm.get_init_feat(obs, state, sample=self._config.collect_dyn_sample and not self._config.eval_state_mean)
        if not training:
            goal = self._wm.get_goal(obs, training=False)
            if self._config.offpolicy_opt:
                if self._config.offpolicy_use_embed:
                    action = self._off_policy_handler.actor(torch.cat([self._wm.encoder(obs), goal], dim=-1))
                else:
                    action = self._off_policy_handler.actor(torch.cat([obs['image'], obs['image_goal']], dim=-1))
                reward = torch.zeros((action.shape[0], 1), dtype=torch.float32)
            else:
                action = self._task_behavior.act(feat, obs, latent).mode()
                actor_inp = torch.cat([feat, goal], dim=-1)
                reward = self._task_behavior._gc_reward(actor_inp.unsqueeze(0), latent, action, obs['image_goal'].unsqueeze(0))
            ...
            if self._config.offpolicy_use_embed:
                action = self._off_policy_handler.actor(torch.cat([self._wm.encoder(obs), self._wm.encoder({'image': obs['image_goal']})], dim=-1))
            else:
                action = self._off_policy_handler.actor(torch.cat([obs['image'], obs['image_goal']], dim=-1))
        else:
            action = self._task_behavior.act(feat, obs, latent).sample()
        if self._config.actor_dist == 'onehot_gumble':
            action = F.one_hot(torch.argmax(action, dim=-1), self._config.num_actions).to(action.dtype)
    ...
    def _exploration(self, action, training):
        ...
        if amount == 0:
            return action
        amount = torch.tensor(amount, dtype=self._float)
    ...
    def _get_imag_data(self, data, start):
        ...
        imag_feat, imag_state, imag_action = self._task_behavior._imagine(start, self._task_behavior.actor, self._config.imag_horizon, goal=goal)
...
def make_dataset(episodes, config):
    ...
    generator = lambda: tools.sample_episodes(episodes, config.batch_length, config.oversample_ends)
    dataset = DataLoader(IterableDataset(generator), batch_size=config.batch_size, drop_last=True)
    return dataset
...
def make_env(config, logger, mode, train_eps, eval_eps, use_goal_idx=False, log_per_goal=False):
    if 'dmc' in config.task:
        ...
        dmc_envs = [envs.DmcEnv(task, config.size, config.action_repeat, use_goal_idx, log_per_goal) for task in ['walker_walk', 'quadruped_run']]
        env = envs.MultiplexedEnv([kitchen_env, robobin_env] + dmc_envs, config.action_repeat, config.size, use_goal_idx, log_per_goal)
        env = envs.PadActions(env, [env.action_space for env in env.envs])
        env = wrappers.NormalizeActions(env)
    else:
        raise NotImplementedError(config.task)
    env = wrappers.TimeLimit(env, config.time_limit)
    callbacks = [functools.partial(process_episode, config, logger, mode, train_eps, eval_eps)]
    env = wrappers.CollectDataset(env, callbacks)
    env = wrappers.RewardObs(env)
    return env

def process_episode(config, logger, mode, train_eps, eval_eps, episode):
    directory = {'train': config.traindir, 'eval': config.evaldir}[mode]
    cache = {'train': train_eps, 'eval': eval_eps}[mode]
...
def main(logdir, config):
    ...
    state = None
    while agent._step.item() < config.steps:
        logger.write()
